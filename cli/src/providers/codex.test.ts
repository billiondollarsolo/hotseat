...
describe('CodexProvider', () => {
  ...
  describe('spawn', () => {
    it('should spawn codex process with correct arguments', async () => {
      ...
      expect(spawn).toHaveBeenCalledWith(
        'codex',
        expect.arrayContaining(['exec', '--json', 'You are a helpful assistant']),
  ...
  describe('send', () => {
    ...
    it('should spawn new process for each message (codex exec pattern)', async () => {
      ...
      expect(spawn).toHaveBeenCalledWith(
        'codex',
        expect.arrayContaining(['exec', '--json', 'Hello, Codex!']),
        expect.objectContaining({
          stdio: ['ignore', 'pipe', 'pipe'],
        })
      );
    ...
    it('should resume the Codex thread when one is known', async () => {
      provider.setSessionState({ codexThreadId: 'thread-123' });

      await provider.send({ content: 'Follow up' });

      expect(spawn).toHaveBeenCalledWith(
        'codex',
        expect.arrayContaining(['exec', 'resume', '--json', 'thread-123', 'Follow up']),
        expect.objectContaining({
          stdio: ['ignore', 'pipe', 'pipe'],
        })
      );
  ...
  describe('receive', () => {
    ...
    it('should accumulate item.completed message text until turn completion', async () => {
      const receivePromise = provider.receive();

      mockProcess._stdoutEmitter.emit(
        'data',
        Buffer.from(
          '{"type":"item.completed","item":{"type":"agent_message","text":"Streaming content..."}}\n'
        )
      );
      mockProcess._stdoutEmitter.emit(
        'data',
        Buffer.from('{"type":"turn.completed","usage":{"input_tokens":1}}\n')
      );
    ...
    it('should ignore item.started message until turn completion', async () => {
      const receivePromise = provider.receive();
      ...
      mockProcess._stdoutEmitter.emit(
        'data',
        Buffer.from('{"type":"turn.completed","text":"Done"}\n')
      );
    ...
    it('should store thread.started id and wait for turn completion', async () => {
      const receivePromise = provider.receive();

      mockProcess._stdoutEmitter.emit(
        'data',
        Buffer.from('{"type":"thread.started","thread_id":"thread-123"}\n')
      );
      mockProcess._stdoutEmitter.emit(
        'data',
        Buffer.from('{"type":"turn.completed","text":"Started"}\n')
      );

      const response = await receivePromise;

      expect(response.content).toBe('Started');
      expect(response.isComplete).toBe(true);
      expect(provider.getSessionState()).toEqual({ codexThreadId: 'thread-123' });
    });

    it('should ignore turn.started message until turn completion', async () => {
      const receivePromise = provider.receive();
      ...
      mockProcess._stdoutEmitter.emit(
        'data',
        Buffer.from('{"type":"turn.completed","text":"Turn done"}\n')
      );

      const response = await receivePromise;

      expect(response.content).toBe('Turn done');
      expect(response.isComplete).toBe(true);
    });
    ...
    it('should return accumulated streaming content when turn completes', async () => {
      const receivePromise = provider.receive();
      mockProcess._stdoutEmitter.emit(
        'data',
      ...
      const response = await receivePromise;
      expect(response.content).toBe('First chunkSecond chunk');
      expect(response.isComplete).toBe(true);
    });
