...
export class CodexProvider extends BaseProvider {
  ...
  async spawn(systemPrompt: string): Promise<void> {
    ...
    const args = this.buildExecArgs(systemPrompt);
    ...
    for (const line of lines) {
      ...
      try {
        ...
        if (parsed && this.outputBuffer.resolve) {
          ...
          // Resolve only once a full Codex turn is complete. Intermediate events
          // update state or accumulate text but are not useful Hotseat responses.
  ...
  private parseCodexEvent(line: string): ProviderResponse | null {
    ...
    if (event.type === 'thread.started') {
      this.threadId = event.thread_id ?? event.id ?? this.threadId;
      return null;
    }
  ...
  async send(message: ProviderMessage): Promise<void> {
    ...
    const args = this.buildExecArgs(message.content);
  ...
  /**
   * Get the persisted Codex session state.
   */
  getSessionState(): ProviderSessionState | null {
    return this.threadId ? { codexThreadId: this.threadId } : null;
  }

  /**
   * Restore a previously saved Codex session id.
   */
  setSessionState(state: ProviderSessionState | null): void {
    this.threadId = typeof state?.codexThreadId === 'string' ? state.codexThreadId : null;
  }

  /**
   * Build Codex exec args. New conversations use `codex exec`; subsequent
   * turns use `codex exec resume <thread>` so Codex goals and context persist.
   */
