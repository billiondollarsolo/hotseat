...
describe('createProgram', () => {
  it('creates a Commander program with correct name', () => {
    ...
    expect(program.name()).toBe('hotseat');
  });
  ...
  it('has the correct description', () => {
    ...
    expect(description).toContain('Hotseat conducts interactive interviews');
  ...
  it('includes detailed description', () => {
    ...
    expect(description).toContain('Product Requirements Documents');
    expect(description).toContain('./hotseat/');
  });

  it('EXAMPLES constant demonstrates all features', () => {
    expect(EXAMPLES).toContain('Examples:');
    expect(EXAMPLES).toContain('hotseat "user authentication system"');
  ...
  it('has clear descriptions for each option', () => {
    ...
    expect(resumeOpt?.description).toContain('./hotseat/state.yaml');
...
describe('runCLI', () => {
  ...
  it('parses feature description from arguments', () => {
    const result = runCLI(['node', 'hotseat', 'add user authentication']);
  ...
  it('handles --resume flag', () => {
    const result = runCLI(['node', 'hotseat', '--resume']);
  ...
  it('returns error when no feature provided and no --resume', () => {
    const result = runCLI(['node', 'hotseat']);
  ...
  it('parses --first-principles flag', () => {
    const result = runCLI(['node', 'hotseat', 'feature', '--first-principles']);

    expect(result.options.firstPrinciples).toBe(true);
  });

  it('parses --provider option', () => {
    const result = runCLI(['node', 'hotseat', 'feature', '--provider', 'claude']);
  ...
  it('uses default provider when not specified', () => {
    const result = runCLI(['node', 'hotseat', 'some feature']);
...
describe('EXAMPLES', () => {
  ...
  it('contains usage examples with $ hotseat prefix', () => {
    expect(EXAMPLES).toContain('$ hotseat');
  });
...
describe('getHelpText', () => {
  ...
  it('includes program name', () => {
    const helpText = getHelpText();
    expect(helpText).toContain('hotseat');
  });
