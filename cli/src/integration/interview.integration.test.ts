...
describe('Interview Flow Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = join(tmpdir(), `hotseat-integration-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  describe('State Resume Functionality', () => {
    ...
    it('should handle corrupted state gracefully', async () => {
      ...
      const hotseatDir = join(testDir, 'hotseat');
      await mkdir(hotseatDir, { recursive: true });
      const { writeFile } = await import('node:fs/promises');
      await writeFile(join(hotseatDir, 'state.yaml'), 'invalid: yaml: content: [[[');
  ...
  describe('Error Recovery', () => {
    ...
    it('should provide recovery instructions for recoverable errors', () => {
      ...
      expect(instructions).toContain('hotseat --resume');
    });
  });
