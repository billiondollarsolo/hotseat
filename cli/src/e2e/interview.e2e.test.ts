/**
 * End-to-End Tests for Hotseat CLI with Real AI Providers
 *
 * These tests verify the E2E functionality of Hotseat with real AI CLI tools.
...
describe('E2E Tests with Real AI CLIs', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = join(
      tmpdir(),
      `hotseat-e2e-test-${Date.now()}-${Math.random().toString(36).slice(2)}`
    );
  ...
  describe('PRD File Generation (E2E)', () => {
    it('should generate PRD files with correct structure', async () => {
      ...
      console.log(`    PRD files generated at ${testDir}/hotseat/`);
    });
...
describe('Structured Response Parsing (E2E)', () => {
  it('should have correct structured markers defined', () => {
    expect(STRUCTURED_MARKERS.questionStart).toBe('<<<HOTSEAT_QUESTION>>>');
    expect(STRUCTURED_MARKERS.questionEnd).toBe('<<<END_HOTSEAT_QUESTION>>>');
    expect(STRUCTURED_MARKERS.completeStart).toBe('<<<HOTSEAT_COMPLETE>>>');
    expect(STRUCTURED_MARKERS.completeEnd).toBe('<<<END_HOTSEAT_COMPLETE>>>');
  });
...
<<<END_HOTSEAT_COMPLETE>>>`;
  ...
  it('should handle multi-select question', () => {
    const mockContent = `<<<HOTSEAT_QUESTION>>>
{
  "header": "Features",
...
More text`;
  ...
  it('should handle incomplete structured markers', () => {
    const mockContent = `Some text <<<HOTSEAT_QUESTION>>> but no end marker`;
