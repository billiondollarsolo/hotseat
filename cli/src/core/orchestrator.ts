...
export const STRUCTURED_MARKERS = {
  questionStart: '<<<HOTSEAT_QUESTION>>>',
  questionEnd: '<<<END_HOTSEAT_QUESTION>>>',
  completeStart: '<<<HOTSEAT_COMPLETE>>>',
  completeEnd: '<<<END_HOTSEAT_COMPLETE>>>',
} as const;
...
export function generateSystemPrompt(config: OrchestratorConfig): string {
  ...
  parts.push(`You are Hotseat, an AI assistant that helps developers plan software features through structured interviews.
...
If Codex goal tools are available, create one active goal for this interview: "Complete Hotseat PRD interview for: ${config.feature}". Keep that goal active while gathering requirements, and mark it complete only after you emit the final Hotseat completion block.
`);
  }
...
export class InterviewOrchestrator {
  ...
  setState(state: InterviewState): void {
    this.state = state;
    this.config.provider.setSessionState?.(state.providerSession ?? null);
  }
  ...
  /**
   * Resume an existing provider-backed session from restored state.
   */
  async resume(userPrompt: string): Promise<TurnResult> {
    if (this.isInitialized) {
      throw new Error('Orchestrator already initialized');
    }

    this.isInitialized = true;
    return this.sendUserResponse(userPrompt);
  }
  ...
  /**
   * Copy provider-specific session data into interview state before saving.
   */
  private syncProviderSession(): void {
    const providerSession = this.config.provider.getSessionState?.() ?? null;
    if (providerSession) {
      this.state = {
        ...this.state,
        providerSession,
...
export function createOrchestratorFromState(
  ...
  provider.setSessionState?.(state.providerSession ?? null);
