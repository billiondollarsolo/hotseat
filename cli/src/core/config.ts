/**
 * Configuration management for Hotseat CLI
 * Handles loading, saving, and validating configuration from ./hotseat/config.yaml
 */
...
export function getConfigPath(baseDir: string = process.cwd()): string {
  return join(baseDir, 'hotseat', 'config.yaml');
}
...
export async function createDefaultConfig(configPath: string): Promise<void> {
  ...
  const contentWithComments = `# Hotseat CLI Configuration
...
export async function loadConfig(baseDir: string = process.cwd()): Promise<ConfigResult> {
  ...
  return {
    config: parsed as HotseatConfig,
    wasCreated,
    configPath,
  };
}
 ...
 */
export async function saveConfig(config: HotseatConfig, baseDir: string = process.cwd()): Promise<void> {
  const configPath = getConfigPath(baseDir);
  const dir = dirname(configPath);
 ...
 */
export function getDefaultConfig(): HotseatConfig {
