...
describe('state', () => {
  ...
  describe('getStatePath', () => {
    it('returns correct path for given base directory', () => {
      ...
      expect(path).toBe('/some/project/hotseat/state.yaml');
    });

    it('uses current directory when no base specified', () => {
      const path = getStatePath();
      expect(path).toContain('hotseat/state.yaml');
    });
  });
  ...
  describe('hasState', () => {
    ...
    it('returns true when state file exists', async () => {
      const stateDir = join(testDir, 'hotseat');
...
`;
      ...
      expect(result!.statePath).toBe(join(testDir, 'hotseat', 'state.yaml'));
    });
...
`;
  ...
  describe('saveState', () => {
    it('saves valid state to disk', async () => {
      ...
      expect(path).toBe(join(testDir, 'hotseat', 'state.yaml'));
    ...
    it('creates directory if not exists', async () => {
      ...
      const content = await readFile(join(testDir, 'new-project', 'hotseat', 'state.yaml'), 'utf-8');
      expect(content).toContain('feature: Test feature');
    });
    ...
    it('updates updatedAt timestamp', async () => {
      ...
      const content = await readFile(join(testDir, 'hotseat', 'state.yaml'), 'utf-8');
    ...
    it('includes helpful header', async () => {
      ...
      const content = await readFile(join(testDir, 'hotseat', 'state.yaml'), 'utf-8');
      expect(content).toContain('# Hotseat Interview State');
