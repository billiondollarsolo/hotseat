/**
 * State management for Hotseat CLI
...
import type { ProviderName, ProviderSessionState } from '../providers/index.js';
...
export interface InterviewState {
  ...
  aiContext: string;
  /** Provider-specific session data used to resume provider context */
  providerSession?: ProviderSessionState;
}
...
export function getStatePath(baseDir: string = process.cwd()): string {
  return join(baseDir, 'hotseat', 'state.yaml');
}
...
export function validateState(state: unknown): StateValidationError[] {
  ...
  // Validate providerSession if present
  if (
    s.providerSession !== undefined &&
    (typeof s.providerSession !== 'object' || s.providerSession === null || Array.isArray(s.providerSession))
  ) {
    errors.push({ field: 'providerSession', message: 'providerSession must be an object' });
  }
...
export async function saveState(state: InterviewState, baseDir: string = process.cwd()): Promise<string> {
  ...
  // Add helpful header
  const contentWithHeader = `# Hotseat Interview State
