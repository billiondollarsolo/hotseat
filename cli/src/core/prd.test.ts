...
describe('PRD Generator', () => {
  ...
  beforeEach(async () => {
    testDir = join(tmpdir(), `hotseat-prd-test-${Date.now()}`);
  ...
  describe('generateMarkdown', () => {
    ...
    it('includes generation date', () => {
      ...
      expect(markdown).toContain(`Generated by Hotseat CLI on ${today}`);
    });
  ...
  describe('getPRDPath', () => {
    it('returns default path with no options', () => {
      ...
      expect(path).toBe(join('/project', 'hotseat', 'my-feature.md'));
    ...
    it('respects baseDir option', () => {
      ...
      expect(path).toBe(join('/custom', 'hotseat', 'my-feature.md'));
    });
    ...
    it('generates json extension', () => {
      ...
      expect(path).toBe(join('/project', 'hotseat', 'my-feature.json'));
    });
  });
  ...
  describe('writePRDMarkdown', () => {
    it('writes markdown file to disk', async () => {
      ...
      expect(result.markdownPath).toBe(join(testDir, 'hotseat', 'test-feature.md'));
  ...
  describe('generateJSON', () => {
    it('generates complete JSON structure', () => {
      ...
      expect(json.$schema).toBe('https://hotseat-cli.dev/schemas/prd-v1.json');
      expect(json.version).toBe('1.0.0');
      expect(json.metadata.slug).toBe('user-auth');
      expect(json.metadata.generator).toBe('hotseat-cli');
  ...
  describe('writePRDJSON', () => {
    it('writes JSON file to disk', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.jsonPath).toBe(join(testDir, 'hotseat', 'test-feature.json'));
