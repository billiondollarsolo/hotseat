...
export interface PRDGeneratorOptions {
  ...
  baseDir?: string;
  /** Output directory relative to baseDir (defaults to ./hotseat) */
  outputDir?: string;
...
export function generateMarkdown(prd: PRDData, slug: string, featureName?: string): string {
  ...
  lines.push(`# ${title}`);
  lines.push('');
  lines.push(`> Generated by Hotseat CLI on ${new Date().toISOString().split('T')[0]}`);
  lines.push('');
...
export function getPRDPath(slug: string, extension: 'md' | 'json', options: PRDGeneratorOptions = {}): string {
  ...
  const outputDir = options.outputDir || 'hotseat';
