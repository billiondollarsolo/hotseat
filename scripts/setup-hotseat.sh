...
while [[ $# -gt 0 ]]; do
  case $1 in
    -h|--help)
      cat << 'HELP_EOF'
Hotseat Plan - Interactive specification gathering workflow
...
DESCRIPTION:
  ...
  Use --first-principles to have Hotseat challenge your assumptions first,
...
OUTPUT:
  ...
  Draft maintained at: .claude/hotseat-draft.md

WORKFLOW:
  1. Hotseat plans - Generate spec: /hotseat:plan "my feature"
...
if [[ -z "$FEATURE_NAME" ]]; then
  ...
  echo "" >&2
  echo "   Example: /hotseat:plan \"user authentication\"" >&2
  exit 1
fi
...
DRAFT_PATH=".claude/hotseat-draft.md"
STATE_PATH=".claude/hotseat-${FEATURE_SLUG}.md"
...
# Hotseat Plan Interview Session
...
echo "Hotseat Plan - Interview Started"
echo ""
